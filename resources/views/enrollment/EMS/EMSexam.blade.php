...
<!-- 主畫面 -->
  ...
  <section class="container"><br>
    ...
    <div class="row">
      ...
      <div id="maincontent">
        @if($auth)
        <form method="post" action="{{ url('/enrollment/EMS/EMSexam') }}" enctype="multipart/form-data">
            {{ csrf_field() }}
          ...
          @endif
          @foreach($EMSexam as $pdf)
          <embed class="view" src ="{{ asset('enrollmentpdf/emsexam/'.$pdf->pdf_name) }}"></embed>
          @if($auth)
          <form action="{{ url('/enrollment/EMS/EMSexam/'.$pdf->id) }}" method="post">
          {{ csrf_field() }}
