	...
	<div class="row">
    	...
    	<div class="col-sm-9 contentAreaStyle"><br>
			...
			<div id="maincontent">
                ...
                @if($post->updated_at)
                <div style="text-align:right;" style="font-size: 13px">{{ $post->updated_at->format('Y-m-d') }}</div> 
                @endif
                ...
                <div id="content" style="display:none;" >
                    ...
                    <form action="{{ url('enrollment/'.$post->id.'/post') }}" method="POST">
                        @csrf
                ...
                <a href="{{ asset('download/enrollment_post/'.$post->filename) }}">
                    {!! $post->filename !!}
                </a><br><br>
                
                @if($auth)
                    @if(!$post->filename)
                        <form action="{{ url('enrollment/'.$post->id.'/storefile')}}" method="POST" enctype="multipart/form-data">
                    ...
                    @endif
                    @if($post->filename)
                        <form action="{{ url('enrollment/'.$post->id.'/deletefile') }}" method="post">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
