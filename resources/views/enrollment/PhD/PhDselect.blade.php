...
<!-- 主畫面 -->
  ...
  <section class="container"><br>
    ...
    <div class="row">
      ...
      <div id="maincontent">
        @if($auth)
        <form method="post" action="{{ url('/enrollment/PhD/PhDselect') }}" enctype="multipart/form-data">
            {{ csrf_field() }}
          ...
          @endif
          @foreach($PhDselect as $pdf)
          <embed class="view" src ="{{ asset('enrollmentpdf/phdselect/' . $pdf->pdf_name) }}"></embed>
          @if($auth)
          <form action="{{ url('/enrollment/PhD/PhDselect/'.$pdf->id) }}" method="post">
          {{ csrf_field() }}
