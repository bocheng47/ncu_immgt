...
<!-- 主畫面 -->
  ...
  <section class="container"><br>
    ...
    <div class="row">
      ...
      <div id="maincontent">
        @if($auth)
        <form method="post" action="{{ url('/enrollment/MS/MSexam') }}" enctype="multipart/form-data">
            {{ csrf_field() }}
          ...
          @endif
          @foreach($MSexam as $pdf)
          <embed class="view" src ="{{ asset('enrollmentpdf/msexam/' . $pdf->pdf_name) }}"></embed>
          @if($auth)
          <form action="{{ url('/enrollment/MS/MSexam/'.$pdf->id) }}" method="post">
          {{ csrf_field() }}
