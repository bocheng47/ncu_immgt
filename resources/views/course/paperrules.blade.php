			...
			<div class="col-sm-9 contentAreaStyle">
				...
				{{-- if the user's been authenticated ends. 0 = admins, 1 = teachers. --}}
					...
					<table class="content-table" style="width: 100%;">
						...
						<tbody>
							@php ($i = 1)
					  		@foreach($paperrules as $paperrule)
							    <tr class="data-row" data-url="{{ url('/download/course/paperrules/'.$paperrule->filename) }}" data-id="{{ $paperrule->id }}">
							      	<td class="iteration"><a class="anchor" id="{{$paperrule->id}}"></a>{{ $i }}</td>
							      	<td class="course-Title">{{ $paperrule->title }}</td>
							      	@if($auth)
							      		<td><button type="button" class="btn btn-warning edit" id="edit-item" data-item-id="{{ $i }}">修改</button></td>
							      		<td>
							      			<form onsubmit="return confirm('您確定要刪除嗎?');" action="{{ url('course/paperrule/'.$paperrule->id) }}" method="POST" style="display: inline;">
											    @method('delete')
											    @csrf
								      	...
								      	<td>
								      		<button  class="btn btn-social-icon btn-facebook" data-title="{{ $paperrule->title }}" data-description="下載" data-url="{{ url('/course/paperrule#'.$paperrule->id) }}" data-social="facebook" ><span class="fa fa-facebook"></span><span data-counter="facebook"></span></button>
								      	</td>
								    @endif
							    </tr>
							    @php ($i++)
...
@if($auth)
<script type="text/javascript" src="{{ asset('js/course/paperrule.admin.EUi6aaVo8HGeO7Oc.js') }}"></script>
